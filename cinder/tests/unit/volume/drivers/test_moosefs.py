...
from cinder.volume.drivers import moosefs
...
class MoosefsTestCase(test.TestCase):
    ...
    def setUp(self):
        super(MoosefsTestCase, self).setUp()

        self._cfg = mock.MagicMock()
        self._cfg.moosefs_shares_config = '/fake/config/path'
        self._cfg.moosefs_sparsed_volumes = False
        self._cfg.moosefs_used_ratio = 0.7
        self._cfg.moosefs_mount_point_base = self._FAKE_MNT_BASE
        self._cfg.moosefs_default_volume_format = 'raw'
        self._cfg.nas_secure_file_operations = 'auto'
        self._cfg.nas_secure_file_permissions = 'auto'

        self._mfs_driver = moosefs.MoosefsDriver(configuration=self._cfg)
        self._mfs_driver._local_volume_dir = mock.Mock(
            return_value=self._FAKE_MNT_POINT)
        self._mfs_driver._execute = mock.Mock()
        self._mfs_driver.base = self._FAKE_MNT_BASE
    ...
    def _path_exists(self, path):
        if path.startswith(self._cfg.moosefs_shares_config):
            return True
    ...
    def test_setup_no_mfsmount(self, mock_exists):
        ...
        self._mfs_driver._execute.side_effect = exc
        self.assertRaises(exception.MoosefsException,
                          self._mfs_driver.do_setup,
                          mock.sentinel.context)
    ...
    def test_ensure_share_mounted_invalid_share(self):
        self.assertRaises(exception.MoosefsException,
                          self._mfs_driver._ensure_share_mounted, ':')
